...
// suffix it truncates at. Both needles are needed. A component folding to ".."
// aliases the parent directory, and one folding to "." aliases the directory it
// sits in, which turns "refs/heads/<U+200C>./main" into "refs/heads/main".
//
// A component holding no '.' can match none of them, so the fold checks run
// only when the control-character scan saw one. The checks run regardless of
// the host OS, because a name can be authored on one system and reach the
...
func HasUnsafeComponent(name string) bool {
	for _, part := range strings.FieldsFunc(name, isPathSep) {
		...
		if IsHFSDot(part, "") || IsNTFSDot(part, "", "") ||
